...
Dict keys not in the whitelist are pruned; list elements are recursed into (pruned by parent path).
"""
...
class JSONPrunePlugin(Plugin):
    ...
    def _prune_string(self, payload: ToolPostInvokePayload, whitelist: list[str]) -> ToolPostInvokeResult:
        ...
        return ToolPostInvokeResult(
            modified_payload=ToolPostInvokePayload(name=payload.name, result="{}"),
            metadata={"pruned": True},
        )
